...
export default function HealthPanel({ health }) {
  ...
  return (
    <div className="bg-secondary/50 border border-border rounded-lg p-4 h-full flex flex-col card-hover">
      ...
      <div className="grid grid-cols-2 gap-2 flex-1">
        {metrics.map((metric, index) => (
          <motion.div
            ...
            className="bg-background/50 rounded-lg p-2.5 border border-border flex flex-col justify-center"
          >
            <div className="text-muted-foreground text-[10px] mb-1">
              {metric.label}
            </div>
            <div className="text-foreground text-lg font-medium mb-0.5">
              {metric.value}
            </div>
