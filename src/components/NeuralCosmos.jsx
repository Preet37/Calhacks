  ...
  const updateExistingNodes = (svg, pipelineSpec, width, height, simulation) => {
    ...
    nodeGroups.selectAll('circle')
      ...
      .attr('stroke', function(d) {
        ...
        if (!parent) return getLatencyColor(d?.latency_ms)
    ...
    nodeGroups.selectAll('text')
      ...
      .text(function() {
        ...
        if (classList && classList.includes('status-label')) {
          if (parent.status === 'running') return '⟳ Running'
          if (parent.status === 'failed') return '✕ Failed'
          if (parent.status === 'pending') return '○ Pending'
          return parent.type === 'http' ? 'API' : 'Transform'
        }
  ...
  useEffect(() => {
    ...
    node.append('circle')
      ...
      .attr('fill', d => {
        ...
        return getLatencyColor(d.latency_ms)
      })
      .attr('opacity', d => {
  ...
  return (
    <div className="relative w-full h-full bg-zinc-950">
      ...
      <div className="absolute bottom-4 left-4">
        <kbd className="bg-zinc-900/90 backdrop-blur px-2 py-1 rounded text-[10px] text-zinc-400 border border-zinc-800">
